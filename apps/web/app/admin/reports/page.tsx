...
export default function AdminReportsPage() {
  ...
  const [clinicProfile, setClinicProfile] = useState({
    name: 'FurEver Paw Care Veterinary Clinic & Pet Hospital',
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // 1. Fetch Pet Monitoring reports (ADMIN role)
        const monRes = await fetch('/api/monitoring?role=ADMIN');
    ...
    // Check clinic info
    const storedAdmin = localStorage.getItem('adminPersonalInfo');
    if (storedAdmin) {
      try {
  ...
  const handleAddFindings = async () => {
    ...
    const finding = prompt("Enter Clinic Admin observation or clinical findings:");
    if (finding) {
      try {
        ...
        alert('Clinic Admin findings successfully updated.');
      } catch (e) {
        console.error(e);
      }
    }
        ...
        <div className="report-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setSelectedReport(null); }}>
          <div className="report-modal-container">
            ...
            <div className="report-document-sheet">
              ...
              {/* Staff / Admin Findings */}
              {selectedReport.adminFindings && (
                <div className="doc-findings-box">
                  <strong><i className="fas fa-user-md"></i> Clinic Staff / Admin Findings:</strong><br />
                  {selectedReport.adminFindings}
                </div>
              )}
              ...
              <div className="doc-signature-section">
                <div className="doc-legal-note">
                  This official medical documentation is verified and generated by FurEver Paw Care Veterinary Management System. 
