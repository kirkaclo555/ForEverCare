...
export default function AppointmentPage() {
  ...
  const [appointmentToApproveCancel, setAppointmentToApproveCancel] = useState<string | null>(null);
  ...
  const handleSaveAppointment = async (e: React.FormEvent) => {
    ...
    const selectedUser = users.find(u => u.id === selectedUserId);
    const ownerName = selectedUser?.fullName || selectedUser?.name || ownerSearch.trim() || 'Unknown';
    const selectedPet = pets.find(p => p.id === selectedPetId);
  ...
  const handleUpdatePrices = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setPricingSuccessMsg('Error: Failed to update prices: ' + (data.error || 'Unknown error'));
      }
    } catch (err) {
      console.error('Error updating prices:', err);
      setPricingSuccessMsg('Error: Error updating prices');
    } finally {
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    if (updatedApp && updatedApp.type === 'telemedicine' && newStatus === 'confirmed') {
        setSuccessMessage(`SMS Successfully Sent to ${updatedApp.owner} at ${updatedApp.contact}:\n\n"Your Telemedicine session is confirmed. Your unique session code is: ${updatedApp.sessionCode}"`);
    }
    setOpenActionMenuId(null);
  };

  const handleApproveCancellation = (id: string) => {
    setAppointmentToApproveCancel(id);
    setOpenActionMenuId(null);
  };

  const confirmApproveCancellation = async () => {
    if (appointmentToApproveCancel) {
      const id = appointmentToApproveCancel;
      setAppointmentToApproveCancel(null);
      try {
        const res = await fetch(`/api/appointments/${id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ approveCancel: true })
        });
        const data = await res.json();
        if (data.success) {
          setShouldReloadOnClose(true);
          setSuccessMessage("Cancellation approved and refund initiated. The user has been emailed.");
        } else {
          setSuccessMessage("Error: " + (data.error || "Failed to approve cancellation"));
        }
      } catch (err) {
        console.error('Failed to approve cancellation:', err);
        setSuccessMessage("Error: Failed to approve cancellation.");
      }
    }
  };
    ...
    if (appointmentToArchive) {
      try {
        ...
        if (data.success) {
          ...
          setSuccessMessage('Error: ' + (data.error || 'Failed to archive appointment'));
        }
      } catch (err) {
        console.error('Failed to archive appointment:', err);
        setSuccessMessage('Error: Failed to archive appointment.');
      }
    ...
    if (appointmentToDecline) {
      try {
        ...
        if (data.success) {
          ...
          setSuccessMessage('Error: ' + (data.error || 'Failed to decline appointment'));
        }
      } catch (err) {
        console.error('Failed to decline appointment:', err);
        setSuccessMessage('Error: Failed to decline appointment.');
      }
  ...
  return (
    ...
    <div className="module-content" style={{ width: "100%", padding: 0, margin: 0 }}  onClick={closeDropdown}>
        <div className="filters-section">
            ...
            <div style={{ display: 'flex', gap: '15px', flex: 1, justifyContent: 'flex-end', alignItems: 'center' }}>
                ...
                <button className="add-appointment-btn" onClick={() => setIsPricingModalOpen(true)} style={{ background: '#4A5568', margin: 0 }}>
                    <i className="fas fa-cog"></i>
                    Pricing Settings
                </button>
                <button className="add-appointment-btn" onClick={() => { setIsAppointmentModalOpen(true); setFormError(''); }} style={{ margin: 0 }}>
                            ...
                            return (
                                ...
                                {openActionMenuId === app.id && (
                                    <div style={{ position: 'absolute', right: '100%', top: 0, background: 'white', border: '1px solid #e2e8f0', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.12)', zIndex: 10, display: 'flex', flexDirection: 'column', minWidth: '175px', overflow: 'hidden' }}>
                                        {/* Approve Cancellation — only when cancel requested */}
                                        {app.purpose?.startsWith('[CANCEL_REQUESTED]') && (
                                          <button style={{ padding: '9px 14px', textAlign: 'left', border: 'none', background: 'none', cursor: 'pointer', borderBottom: '1px solid #e2e8f0', color: '#E53E3E', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }} onClick={(e) => { e.stopPropagation(); handleApproveCancellation(app.id); }}>
                                            <i className="fas fa-check-circle"></i> Approve Cancel
                                          </button>
                                        )}

                                        {/* Set Pending — only if currently not pending, paid, completed, or declined */}
                                        {!['paid','completed','declined'].includes(app.status.toLowerCase()) && app.status.toLowerCase() !== 'pending' && (
                                          <button style={{ padding: '9px 14px', textAlign: 'left', border: 'none', background: 'none', cursor: 'pointer', borderBottom: '1px solid #e2e8f0', color: '#2d3748', display: 'flex', alignItems: 'center', gap: '8px' }} onClick={(e) => { e.stopPropagation(); handleStatusChange(app.id, 'pending'); }}>
                                            <i className="fas fa-clock" style={{color:'#718096'}}></i> Mark Pending
                                          </button>
                                        )}
                                        ...
                                        {/* Reschedule — only if pending */}
                                        {app.status.toLowerCase() === 'pending' && (
                                          <button style={{ padding: '9px 14px', textAlign: 'left', border: 'none', background: 'none', cursor: 'pointer', borderBottom: '1px solid #e2e8f0', color: '#2E5E3E', display: 'flex', alignItems: 'center', gap: '8px' }} onClick={(e) => { e.stopPropagation(); openRescheduleModal(app); }}>
    ...
    <div className="appointment-modal show" onClick={(e) => { if (e.target === e.currentTarget) { setIsAppointmentModalOpen(false); setErrors({}); setFormError(''); } }}>
        <div className="appointment-modal-content">
            <div className="appointment-modal-header">
                ...
                <button
                    ...
                    onClick={() => { setIsAppointmentModalOpen(false); setErrors({}); setFormError(''); }}
                    title="Close modal"
                >
            ...
            <div className="appointment-modal-body">
                {formError && (
                    <div style={{ background: '#fff5f5', color: '#c53030', padding: '10px 14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fed7d7', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.88rem', fontWeight: '500' }}>
                        <i className="fas fa-exclamation-circle" style={{ fontSize: '1rem' }}></i> {formError}
                    </div>
                )}
                <form id="appointmentForm" onSubmit={handleSaveAppointment}>
                    <div className="appointment-form-grid">
                        ...
                        <div className="appointment-form-group">
                            ...
                            />
                            {pets.length > 0 && (
                                <datalist id="superadmin-pets-list">
                                    {pets.map(p => (
                        ...
                        {/* Optional Payment Details for Super Admin */}
                        <div className="appointment-grid-full" style={{ borderTop: '1px dashed #e2e8f0', paddingTop: '12px', marginTop: '4px' }}>
                            <details style={{ fontSize: '0.85rem' }}>
                                <summary style={{ cursor: 'pointer', fontWeight: 600, color: '#64748b', display: 'flex', alignItems: 'center', gap: '6px', userSelect: 'none' }}>
                                    <i className="fas fa-receipt" style={{ color: '#1a5c3a' }}></i> Payment Details (Optional)
                                </summary>
                                <div className="appointment-form-grid" style={{ marginTop: '14px' }}>
                                    <div className="appointment-form-group">
                                        <label style={{ fontSize: '13px' }}><i className="fas fa-hashtag appointment-label-icon"></i> Reference Number</label>
                                    ...
                                    <div className="appointment-form-group">
                                        <label style={{ fontSize: '13px' }}><i className="fas fa-money-bill-wave appointment-label-icon"></i> Amount Paid</label>
                                    ...
                                    <div className="appointment-form-group appointment-grid-full">
                                        <label style={{ fontSize: '13px' }}><i className="fas fa-image appointment-label-icon"></i> Receipt Image</label>
                                        ...
                                        {receiptImage && (
                                            <div style={{ marginTop: '8px' }}>
                                                <img src={receiptImage} alt="Receipt Preview" style={{ maxWidth: '80px', borderRadius: '6px', border: '1px solid #e2e8f0' }} />
                                            </div>
                                        )}
                                    </div>
                                </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              {['pending', 'paid'].includes(viewAppointmentDetails.status?.toLowerCase()) && (
                <button
                  onClick={() => {
                    const app = viewAppointmentDetails;
                    setViewAppointmentDetails(null);
    ...
    {successMessage && (
        <div className="modal" style={{display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', zIndex: 10000}}>
            <div className="modal-content" style={{background: 'white', padding: '30px', borderRadius: '16px', width: '90%', maxWidth: '400px', textAlign: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.1)'}}>
                <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: successMessage.toLowerCase().includes('error') || successMessage.toLowerCase().includes('fail') ? '#FED7D7' : '#C6F6D5', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px auto' }}>
                    <i className={successMessage.toLowerCase().includes('error') || successMessage.toLowerCase().includes('fail') ? "fas fa-exclamation-circle" : "fas fa-check"} style={{ color: successMessage.toLowerCase().includes('error') || successMessage.toLowerCase().includes('fail') ? '#E53E3E' : '#2E5E3E', fontSize: '2rem' }}></i>
                </div>
                <h3 style={{ margin: '0 0 10px 0', color: '#2d3748', fontSize: '1.4rem' }}>{successMessage.toLowerCase().includes('error') || successMessage.toLowerCase().includes('fail') ? 'Notice' : 'Success!'}</h3>
                ...
                }} style={{ padding: '10px 30px', background: successMessage.toLowerCase().includes('error') || successMessage.toLowerCase().includes('fail') ? '#E53E3E' : '#2E5E3E', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '1rem', width: '100%' }}>OK</button>
            </div>
        </div>
    )}

    {appointmentToApproveCancel && (
        <div className="modal" style={{display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', zIndex: 10000}}>
            <div className="modal-content" style={{background: 'white', padding: '30px', borderRadius: '16px', width: '90%', maxWidth: '450px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)'}}>
                <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: '#FED7D7', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px auto' }}>
                    <i className="fas fa-exclamation-circle" style={{ color: '#E53E3E', fontSize: '2.2rem' }}></i>
                </div>
                <h3 style={{ margin: '0 0 10px 0', color: '#2d3748', fontSize: '1.4rem', textAlign: 'center' }}>Approve Cancellation</h3>
                <p style={{ color: '#718096', marginBottom: '25px', lineHeight: '1.6', textAlign: 'center' }}>
                    Are you sure you want to approve this cancellation request? This will cancel the appointment, trigger the refund process if the payment was verified, and notify the user via email.
                </p>
                <div style={{ display: 'flex', gap: '10px' }}>
                    <button onClick={() => setAppointmentToApproveCancel(null)} style={{ flex: 1, padding: '12px', background: '#edf2f7', color: '#4a5568', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Cancel</button>
                    <button onClick={confirmApproveCancellation} style={{ flex: 1, padding: '12px', background: '#E53E3E', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Yes, Approve</button>
                </div>
            </div>
        </div>
    )}
    {appointmentToArchive && (() => {
      ...
      return (
        ...
        >
          <style>{`
            @keyframes arch-fade-in-sa {
              from { opacity: 0; transform: scale(0.97); }
              to { opacity: 1; transform: scale(1); }
            }
            @media (prefers-reduced-motion: reduce) {
              .arch-modal-card-sa { animation: none !important; }
            }
            .arch-modal-card-sa {
              animation: arch-fade-in-sa 0.2s cubic-bezier(0.22, 1, 0.36, 1);
            }
            .arch-btn-cancel-sa:hover:not(:disabled) {
              background: #f1f4f2 !important;
              border-color: #b5c4bb !important;
            }
            .arch-btn-confirm-sa:hover:not(:disabled) {
              background: #134d30 !important;
            }
            .arch-btn-cancel-sa:focus-visible, .arch-btn-confirm-sa:focus-visible {
            ...
            @media (max-width: 480px) {
              ...
              .arch-btn-cancel-sa, .arch-btn-confirm-sa {
                width: 100% !important;
              }
            }
          `}</style>
          <div
            className="arch-modal-card-sa"
            style={{
              background: '#ffffff',
              borderRadius: '20px',
              width: '100%',
          ...
          >
            ...
            <div style={{ padding: '20px 28px 26px 28px' }}>
              ...
              <div className="arch-btn-row-sa" style={{ display: 'flex', gap: '12px' }}>
                <button
                  type="button"
                  className="arch-btn-cancel-sa"
