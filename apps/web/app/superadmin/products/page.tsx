...
export default function ProductsPage() {
  ...
  const handleDeleteClick = (product: any) => {
    if (window.confirm(`Are you sure you want to completely remove "${product.name}"? This action cannot be undone and will remove the item from all platforms.`)) {
                ...
                <div className="product-cards" id="product-container">
                    {products.map(product => (
                        <div className="product-card" data-category={product.category} data-price={product.price} data-id={product.id} key={product.id}>
                            ...
                            <div className="product-actions" style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                                <button className="edit-btn" onClick={() => handleEditClick(product)} style={{ flex: '1 1 calc(50% - 5px)' }}><i className="fas fa-edit"></i> Edit</button>
                                <button className="sell-btn" onClick={() => handleSellClick(product)} style={{ flex: '1 1 calc(50% - 5px)' }}><i className="fas fa-shopping-cart"></i> Sell</button>
                                <button className="delete-btn" onClick={() => handleDeleteClick(product)} style={{ flex: '1 1 100%', backgroundColor: '#e53e3e', color: 'white', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer', marginTop: '5px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', fontWeight: '500' }}><i className="fas fa-trash"></i> Remove</button>
                            </div>
                        </div>
                    ))}
                </div>
