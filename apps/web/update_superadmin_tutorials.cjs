...
const superadminTutorials = `"use client";
  ...
  const [videos, setVideos] = useState<Video[]>([
    {
      ...
      description: 'A quick tour of the FurEver Paw Care superadmin dashboard.',
    ...
    },
    {
      ...
      description: 'Learn how to approve and decline appointments.',
      url: 'https://www.youtube.com/embed/dQw4w9WgXcQ',
      isLocal: false,
      ...
      <div className="module-content" style={{ width: "100%", padding: 0, margin: 0 }} id="mainContent">
          <div className="tutorials-container">
              ...
              <div className="category-filter">
                  <h3><i className="fas fa-filter"></i> Filter by Category</h3>
                  <div className="category-options" id="categoryOptions">
                      <button className={\`category-btn \${filterCategory === 'all' ? 'active' : ''}\`} onClick={() => setFilterCategory('all')}>All Categories</button>
                      {categories.map(cat => (
                        <button key={cat} className={\`category-btn \${filterCategory === cat ? 'active' : ''}\`} onClick={() => setFilterCategory(cat)}>
                          {cat}
                        </button>
                      ))}
                  </div>
...
fs.writeFileSync('apps/web/app/superadmin/tutorials/page.tsx', superadminTutorials);
console.log("Superadmin tutorials implemented");
